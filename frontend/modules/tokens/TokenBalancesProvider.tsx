...
export function _useTokenBalances(
  ...
  ✅ 查询 token 余额 →
  ✅ 应用滑点和额外 buffer，确保安全 →
  ✅ 把 BigInt 余额格式化成人类可读的格式 →
  ✅ 返回标准化的 TokenAmount[] 列表，用于后续交易、显示或者计算。

  USDC	1,000,000n （表示 1 USDC）
  USDC	990,995n （扣掉了1% + 5n）
   */
  const balances = (tokenBalancesQuery.data || [])
    .map((balance, index) => {
      ...
      let amount = balance.status === 'success' ? (balance.result as bigint) : BigInt(0)
