...
# ❌ usdt-dai  dai-usdt usdt-usdc  usdc-weth (Not considering, not the shortest path literally)
# get the shortest paths from sell_symbol to buy_symbol using bellman-ford
def find_shortest_paths(G: nx.classes.digraph.DiGraph, sell_ID: str, buy_ID: str) -> list:
    # get all nodes that contain the sell symbol
    sell_nodes = [node for node in list(G.nodes) if sell_ID in node]
    # get all nodes that contain the buy symbol
    buy_nodes = [node for node in list(G.nodes) if buy_ID in node]
    # get all paths
    paths = []
    # for each sell node
...
# for usdt to weth:
# Check whether the pool on the path is linked in a chain-like manner, 
# and the sell_id and buy_id are the start and end of the path
# ✅ usdt-dai dai-usdc usdc-weth 
# ❌ usdt-dai usdt-weth usdc-weth
def check_path_validity(G: nx.classes.digraph.Graph, path: list, sell_ID: str, buy_ID: str) -> bool:
    for node in path:
        node_data = G.nodes[node]
        # get the IDs in the node
        ids = [node_data['token0']['id'], node_data['token1']['id']]
...
def validate_all_paths(G: nx.classes.digraph.Graph, paths: list, sell_ID: str, buy_ID: str) -> list:
    valid_paths = []
    for path in paths:
