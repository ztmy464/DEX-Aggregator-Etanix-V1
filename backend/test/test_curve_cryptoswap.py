...
from src.constants import ALCHEMY_KEY
...
from src.slippage.type_pool.curve.CryptoMath import get_y
...
def simulate_exchange_crypto(
    graph_data: dict, # Augmented graph data
    i: int, # Input coin index
    j: int, # Output coin index
    dx_underlying: float, # Amount of input coin (human readable)
) -> float: # Returns estimated dy (human readable)

    n_coins = len(graph_data['coins'])
    balances = graph_data['balances']
    ...
    # --- 2. Handle Input dx ---
    dx_wei = int(dx_underlying * (10**decimals[i]))
    xp = balances[:] # Create copy for calculations
    xp[i] += dx_wei
    print(f"Balances: {balances}")

    # --- 3. Transform Balances ---
    ...
    # --- 5. Calculate dy (Transformed) and Update xp_transformed ---
    ...
    # --- 6. Detransform dy to Underlying Units ---
    ...
    # --- 8. Subtract Fee ---
    dy_final_wei = dy_underlying_wei - fee_amount_wei

    # --- 9. Return Human Readable ---
    dy_final_human = dy_final_wei / (10**decimals[j])

    return dy_final_human

# --- Augment Graph Data and Run Simulation ---
...
graph_data = [pool for pool in filt_pools if pool["address"].lower() == target_address.lower()][0]
contract = web3.eth.contract(address=POOL_ADDRESS, abi=ABI)
graph_data = get_pool_data(graph_data,contract)

# Price Scale
print(f"Price Scale: {graph_data['price_scales']}")
print(f"'D: {graph_data['D']}")
...
# --- Run the Test Case ---
input_token_index = 0  # USDT
output_token_index = 1 # WBTC
input_amount_human = 10000 # 1000 USDT

print(f"\n--- Starting Crypto Simulation: {input_amount_human} {graph_data['coins'][input_token_index]['symbol']} -> {graph_data['coins'][output_token_index]['symbol']} ---")

estimated_output_crypto = simulate_exchange_crypto(
    graph_data=graph_data,
    i=input_token_index,
    j=output_token_index,
    dx_underlying=input_amount_human
)
print(f"Final Output ({graph_data['coins'][output_token_index]['symbol']}): {estimated_output_crypto:.8f}")
